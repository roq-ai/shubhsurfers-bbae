...
import { getProductOrders, deleteProductOrderById } from 'apiSdk/product-orders';
...
function ProductOrderListPage() {
  ...
  const handleView = (id: string) => {
    if (hasAccess('product_order', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/product-orders/view/${id}`);
    }
  };
  ...
  return (
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        {hasAccess('product_order', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <Flex justifyContent="space-between" mb={4}>
            <Text as="h1" fontSize="2xl" fontWeight="bold">
              Product Order
            </Text>
            <NextLink href={`/product-orders/create`} passHref legacyBehavior>
                  ...
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    <Td>
                      {hasAccess('product_order', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                        <NextLink href={`/product-orders/edit/${record.id}`} passHref legacyBehavior>
                          <Button
