...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getUserProducts() {
    const data = await prisma.user_product
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'user_product'));
    return res.status(200).json(data);
  }

  async function createUserProduct() {
    await userProductValidationSchema.validate(req.body);
    const body = { ...req.body };

    const data = await prisma.user_product.create({
      data: body,
    });
    return res.status(200).json(data);
  }
